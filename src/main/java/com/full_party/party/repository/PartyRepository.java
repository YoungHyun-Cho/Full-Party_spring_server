...
public interface PartyRepository extends JpaRepository<Party, Long> {
        ...
        @Query("SELECT p FROM Party p WHERE p.name LIKE %:keyword% AND p.region = :region")
        List<Party> searchPartiesByKeyword(String keyword, String region);

        @Query("SELECT DISTINCT p FROM Party p JOIN p.tagList t WHERE t.value = :tagValue AND p.region = :region")
